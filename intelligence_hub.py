...
def _parse_llava_json_response(raw_text: str) -> dict[str, Any]:
    txt = str(raw_text or "").strip()
    if not txt:
        return {"decisao": "IGNORE", "motivo": "Resposta vazia do modelo de visão."}
    try:
        obj = json.loads(txt)
    ...
    if m:
        try:
            ...
            pass
    return {"decisao": "IGNORE", "motivo": f"JSON inválido na resposta: {txt[:240]}"}


def analyze_chart_with_vision(symbol: str) -> dict[str, Any]:
    """
    Cérebro visual autónomo:
    - lê 50 velas de 15m da Binance Futures;
    - gera candle+volume localmente (mplfinance);
    - envia imagem em base64 ao LLaVA no Replicate;
    - devolve JSON com {"decisao","motivo"}.
    """
    try:
        import executor_futures
    except Exception as e_imp:  # noqa: BLE001
        return {"decisao": "IGNORE", "motivo": f"Falha ao importar executor_futures: {e_imp}"}

    print("[VISION] Gerando gráfico local...", flush=True)
    ...
    # 1) Futures klines (50 velas de 15m)
    rows = ex.fapiPublicGetKlines({"symbol": sid, "interval": "15m", "limit": 50}) or []
    if not isinstance(rows, list) or len(rows) < 20:
        return {"decisao": "IGNORE", "motivo": "Klines insuficientes para análise visual."}
    ...
    if df.empty:
        return {"decisao": "IGNORE", "motivo": "Sem dados válidos para renderizar gráfico."}
    ...
    # 3) Candlestick + volume em memória
    fig = None
    buf = io.BytesIO()
    try:
        fig, _ax = mpf.plot(
        ...
        image_data_uri = f"data:image/png;base64,{b64_img}"
    finally:
        try:
            if fig is not None:
                fig.clf()
    ...
    # 4) Chamada Replicate (LLaVA)
    prompt = (
        "Atuas como um analista quantitativo. Analise este gráfico de candlesticks. "
        "Procure por padrões de Smart Money (Order Blocks, CHoCH, Sweep de Liquidez). "
        'Responda EXCLUSIVAMENTE num formato JSON válido com as chaves: "decisao" '
        '("BUY", "SELL", ou "IGNORE") e "motivo".'
    )
    try:
        out = replicate.run(
            REPLICATE_VISION_MODEL or "yorickvp/llava-13b",
            input={
                "prompt": prompt,
        ...
        result = {"decisao": "IGNORE", "motivo": f"Falha Replicate/LLaVA: {e_rep}"}
        print(f"[VISION] Análise do LLaVA recebida: {result}", flush=True)
        return result

    result = _parse_llava_json_response(str(txt))
    decisao = str(result.get("decisao", "IGNORE")).upper().strip()
    if decisao not in ("BUY", "SELL", "IGNORE"):
        result["decisao"] = "IGNORE"
        result["motivo"] = f"Decisão inválida do modelo: {decisao}"
    print(f"[VISION] Análise do LLaVA recebida: {result}", flush=True)
    return result
