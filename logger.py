...
def _sanitize_payload_keys(payload: dict[str, Any]) -> dict[str, Any]:
    """
    Garante chaves limpas (sem espaços/controles) antes do envio ao Supabase.
    """
    clean: dict[str, Any] = {}
...
def registrar_log_trade(
    ...
    *,
    ...
    payload = _sanitize_payload_keys(dict(dados_log))
    print(f"[DEBUG SUPABASE PAYLOAD] Tentando enviar: {payload}")
    try:
        ...
        print(
            f"[DEBUG SUPABASE ERRO] Tipo: {type(e_first)} | Detalhe completo: {e_first}"
        )
        ...
        for k in _LOGS_OPTIONAL_KEYS_PGRST204_RETRY:
            payload.pop(k, None)
        print(f"[DEBUG SUPABASE PAYLOAD] Tentando enviar (retry PGRST204): {payload}")
        try:
            resposta = _insert_log_row(payload)
            _log_insert_success(resposta)
            print(
            ...
            print(
                f"[DEBUG SUPABASE ERRO] Tipo: {type(e_second)} | Detalhe completo: {e_second}"
            )
    ...
    basico = _sanitize_payload_keys(basico)
    print(f"[DEBUG SUPABASE PAYLOAD] Tentando enviar (fallback básico): {basico}")
    try:
        ...
        print(
            f"[DEBUG SUPABASE ERRO] Tipo: {type(e_final)} | Detalhe completo: {e_final}"
        )
        print(
